...
const EditProduct = ({
  ...
  return (
    <div className={`fixed top-0 left-0 bottom-0 right-0 bg-[rgba(0,0,0,.8)] flex items-center justify-center z-30 transition duration-200 ${openEditProductOverlay ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
      <div ref={editProductOverlayRef} className={`bg-white w-1/3 rounded-xl px-7 py-6 transition duration-200 delay-100 ${openEditProductOverlay ? 'translate-y-0 pointer-events-auto opacity-100' : '-translate-y-12 pointer-events-none opacity-0'}`}>
